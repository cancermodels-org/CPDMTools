#' CTG QC Treatment Plot without Curve Fitting
#'
#' @inheritParams growth_plot_qc_mono
#' @param ctg_data A data frame with CTG data
...
#' @export
#'
ctg_qc_treat_plot_raw <- function(
    ctg_data,
    treat_name = "",
  ...
  treat_count <- ctg_data %>%
    dplyr::count(treatment_name)

  # Add colors
  treat_count <- treat_count %>%
    dplyr::mutate(color = viridis::viridis(nrow(treat_count)))

  # Extract data
  data_long <- ctg_data %>%
    dplyr::filter(treatment_name == treat_name) %>%
    dplyr::select(-color) %>%
    dplyr::left_join(treat_count)

  # Filter on outlier criteria
  ...
  plot_01 <- plot_01 +
    ...
    ggplot2::xlab(
      paste0("Log10(Concentration (", concentration_unit, "))")
    ) +
